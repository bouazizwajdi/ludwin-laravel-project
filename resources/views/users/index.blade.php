    ...
    <table id="kt_datatable_dom_positioning" class="table table-striped table-row-bordered gy-5 gs-7 border rounded">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{$user->email}}</td>
                <td>@if(!empty($user->group)) {{$user->group->name}} @endif</td>
                <td>{{$user->role}}</td>
                <td>{{$user->created_at}}</td>
                <td><a href="#" class="btn btn-sm btn-danger btn-flex btn-center btn-active-light-primary"
                    ...
                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                        ...
                        <div class="menu-item btn-supprimer px-3">
                            <form method="POST" action="{{route("users.destroy",$user->id)}}">
                                ...
                                <button type="submit"
                                    onclick="return confirm('{{__('messages.Are you sure you want to delete?')}}');"
                                    class="menu-link px-3" data-kt-customer-table-filter="Supprimer_row">
                                    {{__('messages.Delete')}}
                                </button>
            ...
            @empty
            <tr>
                <td colspan="2">{{__('messages.no user at the moment')}}</td>
            </tr>
            @endforelse
        </tbody>
