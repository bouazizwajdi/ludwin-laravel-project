...
@section("descmodule",__('messages.Edit Excel File'))
@section("btnright")
...
<div class="card-header pt-7" id="kt_chat_contacts_header">
    <div class="card-title">
        ...
        <h2>  {{__('messages.Edit Excel File')}} </h2>
    </div>
</div>
    ...
    <div class="notice d-flex bg-light-primary rounded border-primary border border-dashed  p-6 m-6">
        ...
        <div class="d-flex flex-stack flex-grow-1 ">
            <div class=" fw-semibold">
                <div class="fs-6 text-gray-700 "> {{__('messages.This form allows you to quickly modify a file')}}</div>
            </div>
...
<div id="kt_account_settings_profile_details" class="collapse show">
    ...
    <form  class="form" action="{{route("excels.update",$file)}}" method="POST" enctype="multipart/form-data">
        @method("PUT")
        @csrf
...
<input class="form-control form-control-lg @error('name') is-invalid @enderror" placeholder="{{__('messages.Enter the excel file name')}}" value="{{old('name',$file->name)}}" type="text" name="name" id="name" required>
